  ...
  <div className="min-h-screen bg-black text-white selection:bg-white selection:text-black">
    ...
    <main>
      <section className="px-6 pt-32 pb-20 text-center max-w-4xl mx-auto">
        ...
        <p className="text-neutral-400 text-xl md:text-2xl mb-12 max-w-2xl mx-auto font-light leading-relaxed">
          The minimal, hyper-fast interface for the world's most capable open-source models.
        </p>
        <div className="flex flex-col md:flex-row items-center justify-center gap-4">
          <button onClick=${onStart} className="w-full md:w-auto bg-white text-black px-8 py-4 rounded-xl text-lg font-bold hover:scale-105 transition-transform">
            Get Started
          </button>
          <button className="w-full md:w-auto border border-white/10 px-8 py-4 rounded-xl text-lg font-bold hover:bg-white/5 transition">
            View Documentation
          </button>
        </div>
      </section>

      <section id="pricing" className="px-6 py-20 bg-neutral-950">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-8">
          ${[
            ...
            <div className=${`p-8 rounded-2xl border ${plan.highlight ? 'border-white bg-white text-black' : 'border-white/10 bg-black text-white'}`}>
              <h3 className="text-lg font-bold mb-2 tracking-tight">${plan.name}</h3>
              <div className="text-4xl font-bold mb-4 tracking-tighter">${plan.price}<span className="text-sm font-normal opacity-50">/mo</span></div>
